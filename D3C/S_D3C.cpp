...
//****
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 Cut1[s].add(Theta1[s+1] - VaR[s]- OptCutExpr1 >= -IloInfinity );
					...
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								Cut1[s][k].setLinearCoef(VaR[s], -lambda*psi_eta[w][k]/(1-alpha)); 
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							Cut1[s-1][k].setLinearCoef(VaR[s-1], -lambda*psi_eta[w][k]/(1-alpha)); 	 
						}//end for k
						...
						Cut1[s-1][nCut].setLinearCoef(VaR[s-1], -lambda*psi_eta[w][nCut]/(1-alpha)); 
