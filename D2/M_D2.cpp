...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
             ...
             if(s<No_stage-1){ 
				 CVaRCons.add(V[s] - OBJ[s] -  lambda*VaR[s] - prob*IloSum(Theta1[s+1]) >= -IloInfinity);
			 }
			 else{
			 ...
			 if(s>0 && s<No_stage-1){
				 ...
				 OBJ[s] += lambda*VaR[s]+ prob*IloSum(Theta1[s+1]);
				 ...
				 for(w = 0; w<No_scen;w++){
					Cut1[s][w] = IloRangeArray(env);
					 ...
					 for(nCut = 0; nCut<MaxCut; nCut++){
						 ...
						 Cut1[s][w].add(Theta1[s+1][w] + OptCutExpr1 + VaR[s]>= -IloInfinity );
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			/*subObj_cx[0] = CPX[0].getObjValue() - lambda*CPX[0].getValue(VaR[0]);
			for(j=0;j<No_scen;j++)
				subObj_cx[0] -=  prob*CPX[0].getValue(Theta1[1][j]); 
*/
			
			subObj_hat[0] = CPX[0].getObjValue() ;
			for(j=0;j<No_scen;j++)
				subObj_hat[0] -=  prob*CPX[0].getValue(Theta1[1][j]); 
	            ...
	            for(w=1; w<No_scenNode; w++){
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								for(j=0;j<No_scen;j++){
									...
									Cut1[s][j][k].setLinearCoef(VaR[s], CutLhsV[w][j][k]);
									for(i=0;i<No_recharge; i++){ 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = subObj_hat[w]/(1-lambda);
							for(j=0;j<No_scen;j++)
								subObj_hat[w] -=  prob*CPX[s].getValue(Theta1[s+1][j]); 
					...
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							for(j=0;j<No_scen;j++){
								...
								Cut1[s-1][j][k].setLinearCoef(VaR[s-1], CutLhsV[w][j][k]);
								for(i=0;i<No_recharge; i++){ 
							...
							for(j=0; j<No_scen; j++){ 
								for(k=0;k<nCut+1;k++){ 
									for(w2 = 0; w2<No_scen; w2++){
										CutRhs1[w][j][nCut] += pi_Cut1[decendant[w][j]][w2][k]*CutRhs1[decendant[w][j]][w2][k];
									}
								} 
							}
						}
					   ...
					   for(j=0; j<No_scen; j++){
						    for(t =0; t<No_year[s];t++){
				               for(i=0; i<No_user; i++){
					              CutRhs1[w][j][nCut] += pi_demand[t][decendant[w][j]][i]*scenFac[decendant[w][j]][t+1]*demand[s][t][i];
							   }
				               for(i=0; i<No_potUser; i++){
					              CutRhs1[w][j][nCut] += pi_return[t][decendant[w][j]][i]*returnRate*scenFac[decendant[w][j]][t+1]*demand[s][t][i];
							   }
						       for(i=1;i<No_capacity; i++){ 
					              CutRhs1[w][j][nCut] += pi_capacity[t][decendant[w][j]][i]*capacity[i];
							   }
							   CutRhs1[w][j][nCut] += pi_capacity[t][decendant[w][j]][0]*CAPamt[s][t]*scenFac[decendant[w][j]][0];
							   for(i=0;i<No_recharge; i++){ 
					              CutRhs1[w][j][nCut] += pi_safe[t][decendant[w][j]][i]*storageUB[i];
							   }
							}
						...
						for(i=0;i<No_recharge; i++){
							for(j=0; j<No_scen; j++){    
								CutLhs1[w][j][nCut][i] = -pi_storage[decendant[w][j]][i]+pi_flowBound[decendant[w][j]][i];
                        ...
                        for(j=0; j<No_scen; j++){    
							CutLhsV[w][j][nCut] = pi_cvar[decendant[w][j]];
							Cut1[s-1][j][nCut].setLinearCoef(VaR[s-1], CutLhsV[w][j][nCut]); 
						}
