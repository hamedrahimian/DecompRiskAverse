			 ...
			 if(s<No_stage-1){
				 for(w = 0; w<No_scen;w++){
					...
					for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 Cut1[s][w].add(Theta1[s+1][w] - VaR[s] - OptCutExpr1 >= -IloInfinity );
						...
						if(s<No_stage-1){ 
							for(k=0;k<nCut;k++){ 
								for(j=0;j<No_scen;j++){
									...
									Cut1[s][j][k].setLinearCoef(VaR[s], -lambda*psi_eta[w][j][k]/(1-alpha)); 
								}
							}//end for nCut
					...
					if(stageMap[w]==s-1){ 
						for(k=0;k<nCut;k++){ 
							for(j=0;j<No_scen;j++){
								...
								Cut1[s-1][j][k].setLinearCoef(VaR[s-1], -lambda*psi_eta[w][j][k]/(1-alpha)); 
							}
						}//end for nCut
						...
						for(j=0;j<No_scen;j++){
							...
							Cut1[s-1][j][nCut].setLinearCoef(VaR[s-1], -lambda*psi_eta[w][j][nCut]/(1-alpha)); 
