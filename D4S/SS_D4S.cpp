...
//****
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 Cut2[s].add(Theta2[s+1] - OptCutExpr1 -VaR[s]>= -IloInfinity );
		...
		IloNumArray2 CutRhs1g(env,No_scenNode); 
		...
		for(w2=0; w2<No_scenNode; w2++){
			...
			CutRhs1g[w2] = IloNumArray(env,MaxCut); 
	            ...
	            for(w=1; w<No_scenNode; w++){
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								Cut1[s][k].setBounds(CutRhs1g[w][k], IloInfinity);
								Cut2[s][k].setBounds(CutRhs2[w][k], IloInfinity);
								for(i=0;i<No_recharge; i++){ 
									Cut1[s][k].setLinearCoef(Storage[i][s][No_year[s]-1], -CutLhs1[w][k][i]);
									Cut2[s][k].setLinearCoef(Storage[i][s][No_year[s]-1], -psi_x[w][k][i]/(1-alpha));
								}
								Cut2[s][k].setLinearCoef(VaR[s], -1-psi_eta[w][k]/(1-alpha)); 
           ...
           for(s = No_stage-1; s>0; s--){
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							Cut1[s-1][k].setBounds(CutRhs1g[w][k], IloInfinity);
							Cut2[s-1][k].setBounds(CutRhs2[w][k], IloInfinity);
							for(i=0;i<No_recharge; i++){ 
								Cut1[s-1][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs1[w][k][i]);
								Cut2[s-1][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][k][i]/(1-alpha));
							}
							Cut2[s-1][k].setLinearCoef(VaR[s-1], -1-psi_eta[w][k]/(1-alpha)); 
                       ...
                       //you have done that before
						...
						//take care of the rest of g
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									g[w][j][nCut] += pi_Cut1[decendant[w][j]][k]*CutRhs1g[decendant[w][j]][k];
									...
									g[w][j][nCut] -= psi_eta[decendant[w][j]][k]*pi_Cut2[decendant[w][j]][k]*var[decendant[w][j]][k]/(1-alpha);
								}

							}
								CutRhs1g[w][nCut]+=prob*g[w][j][nCut];
						}//end j
						...
						for(j=0;j<No_scen;j++){
							...
							if (CutCVaR[w][j]-var[w][nCut] >0){
								psi[w][nCut]+=prob*(CutCVaR[w][j]-var[w][nCut]);
								psi_eta[w][nCut]-=prob;
								for(i=0;i<No_recharge; i++){
						...
						for(i=0;i<No_recharge; i++){
							...
							Cut2[s-1][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][nCut][i]/(1-alpha)); 
						}

						Cut2[s-1][nCut].setLinearCoef(VaR[s-1], -1-psi_eta[w][nCut]/(1-alpha)); 
						
						CutRhs2[w][nCut]+=psi[w][nCut]/(1-alpha);
						for(i=0;i<No_recharge; i++){
							CutRhs2[w][nCut]-=psi_x[w][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
						}
						CutRhs2[w][nCut]-=psi_eta[w][nCut]*var[w][nCut]/(1-alpha);
						Cut1[s-1][nCut].setBounds(CutRhs1g[w][nCut], IloInfinity);
