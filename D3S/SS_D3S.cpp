...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if( s<No_stage-1){ 
				 OBJ[s] +=(1-lambda)*Theta1[s+1] + lambda*(VaR[s]+Theta2[s+1]/(1-alpha)); 
			 }
...
//****
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			subObj_hat[0] = CPX[0].getObjValue() - CPX[0].getValue(Theta1[1])*(1-lambda)-CPX[0].getValue(Theta2[1])*lambda/(1-alpha);
            ...
            for(s=1; s<No_stage;s++){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								for(i=0;i<No_recharge; i++){ 
									...
									Cut2[s][k].setLinearCoef(Storage[i][s][No_year[s]-1], -psi_x[w][k][i]);
								}
								Cut2[s][k].setLinearCoef(VaR[s], -psi_eta[w][k]); 
			   ...
			   // 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue() - CPX[s].getValue(Theta1[s+1])*(1-lambda)-CPX[s].getValue(Theta2[s+1])*lambda/(1-alpha);
					...
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							for(i=0;i<No_recharge; i++){ 
								...
								Cut2[s-1][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][k][i]);
							}
							Cut2[s-1][k].setLinearCoef(VaR[s-1], -psi_eta[w][k]); 
                       ...
                       //you have done that before
						...
						//take care of the rest of g
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									...
									g[w][j][nCut] += psi[decendant[w][j]][k]*pi_Cut2[decendant[w][j]][k];
									for(i=0;i<No_recharge; i++){
										g[w][j][nCut] -= psi_x[decendant[w][j]][k][i]*pi_Cut2[decendant[w][j]][k]*storage_hat[i][decendant[w][j]][k];
									}
									g[w][j][nCut] -= psi_eta[decendant[w][j]][k]*pi_Cut2[decendant[w][j]][k]*var[decendant[w][j]][k];
								}
						...
						for(i=0;i<No_recharge; i++){
							...
							Cut2[s-1][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][nCut][i]); 
						}

						Cut2[s-1][nCut].setLinearCoef(VaR[s-1], -psi_eta[w][nCut]); 
						
						CutRhs2[w][nCut]+=psi[w][nCut];
						for(i=0;i<No_recharge; i++){
							CutRhs2[w][nCut]-=psi_x[w][nCut][i]*storage_hat[i][w][nCut];
						}
						CutRhs2[w][nCut]-=psi_eta[w][nCut]*var[w][nCut];
