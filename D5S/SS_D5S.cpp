...
///******
 ...
 /////////////////// DECISION VARIABLES WITH NAMES  /////////////////////////////
		...
		IloNumVarArray	Theta1(env,No_stage, 0, IloInfinity, ILOFLOAT);
		IloNumVarArray	Theta2(env,No_stage, 0, IloInfinity, ILOFLOAT);
		...
		//between descendant
		IloNumArray2 varIndex(env,No_scenNode);
		//in whole tree
		//IloNumArray2 VarIndex(env,No_scenNode);
		for(w=0;w<No_scenNode;w++){
			varIndex[w] = IloNumArray(env, MaxCut);
			//VarIndex[w] = IloNumArray(env, MaxCut);
			for(nCut=0;nCut<MaxCut;nCut++){
				varIndex[w][nCut] = 0;
				//VarIndex[w][nCut] = 0;
      ...
      //****
		IloNumArray2  pi_Cut1(env, No_scenNode);
		IloNumArray2  pi_Cut2(env, No_scenNode);

		
		
        for(w=0;w<No_scenNode;w++){
			pi_Cut1[w] = IloNumArray(env,MaxCut);
			pi_Cut2[w] = IloNumArray(env,MaxCut);
...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 //Optimality Cut
		 //****
		 EquationArray   Cut1(env, No_stage);
		 EquationArray   Cut2(env, No_stage);
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if( s<No_stage-1){ 
				 OBJ[s] +=(1-lambda)*Theta1[s+1] + lambda*Theta2[s+1]; 
			 }
...
//****
             Cut1[s] = IloRangeArray(env);
			 Cut2[s] = IloRangeArray(env);
//****
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 sprintf_s(name1, "Cut1_%d_%d",(int) s,(int) nCut);
					 					
					 Cut1[s].add(Theta1[s+1] - OptCutExpr1 >= -IloInfinity );
                     Cut1[s][nCut].setName(name1);
					 
					 sprintf_s(name1, "Cut2_%d_%d",(int) s,(int) nCut);
					 Cut2[s].add(Theta2[s+1] - OptCutExpr1>= -IloInfinity );
                     Cut2[s][nCut].setName(name1);
		...
		IloNumArray2 CutRhs2(env,No_scenNode);
        IloNumArray2 CutRhs1(env,No_scenNode); 
		
		IloNumArray3 CutLhs1(env,No_scenNode);
		IloNumArray3 CutLhs2(env,No_scenNode);
		IloNumArray2 CutCVaR(env,No_scenNode);
		IloNumArray4 G(env,No_scenNode);
		IloNumArray3 H(env,No_scenNode);
		IloNumArray3 g(env,No_scenNode);
		IloNumArray2 psi(env,No_scenNode); 
		IloNumArray3 psi_x(env,No_scenNode); 
		IloNumArray2 psi_eta(env,No_scenNode); 
		for(w2=0; w2<No_scenNode; w2++){
			CutRhs2[w2] = IloNumArray(env,MaxCut); 
			CutRhs1[w2] = IloNumArray(env,MaxCut); 
		
			CutLhs1[w2] = IloNumArray2(env,MaxCut);
			CutLhs2[w2] = IloNumArray2(env,MaxCut);
			CutCVaR[w2] = IloNumArray(env,No_scen);
			G[w2] = IloNumArray3(env,No_scen);
			H[w2] = IloNumArray2(env,MaxCut);
			g[w2] = IloNumArray2(env,No_scen);
			psi[w2]=IloNumArray(env, MaxCut);
			psi_x[w2]=IloNumArray2(env, MaxCut);
			psi_eta[w2]=IloNumArray(env, MaxCut);
			...
			for(nCut=0;nCut<MaxCut;nCut++){
				...
				psi[w2][nCut]=0;
				psi_x[w2][nCut]=IloNumArray(env,No_recharge);
				psi_eta[w2][nCut]=0;
				CutLhs1[w2][nCut] = IloNumArray(env,No_recharge);
				CutLhs2[w2][nCut] = IloNumArray(env,No_recharge);
				H[w2][nCut] = IloNumArray(env,No_recharge);
				for(i=0;i<No_recharge;i++){
					CutLhs1[w2][nCut][i] = 0;
					CutLhs2[w2][nCut][i] = 0;
					psi_x[w2][nCut][i]=0;
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			cout<<"nCut ="<<nCut<<endl;
			//cvar: for the positive part of the expression inside expectation
			for(w=0;w<No_scenNode;w++){
				for(j=0;j<No_scen;j++){
					CutCVaR[w][j] = 0;
				}
			...
			subObj_hat[0] = CPX[0].getObjValue() - CPX[0].getValue(Theta1[1])*(1-lambda)-CPX[0].getValue(Theta2[1])*lambda;
            ...
            for(s=1; s<No_stage;s++){ 
	            ...
	            for(w=1; w<No_scenNode; w++){
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								Cut1[s][k].setBounds(CutRhs1[w][k], IloInfinity);
								Cut2[s][k].setBounds(CutRhs2[w][k], IloInfinity);
								for(i=0;i<No_recharge; i++){ 
									Cut1[s][k].setLinearCoef(Storage[i][s][No_year[s]-1], -CutLhs1[w][k][i]);
									Cut2[s][k].setLinearCoef(Storage[i][s][No_year[s]-1], -CutLhs2[w][k][i]);
								}
			   ...
			   // 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue() - CPX[s].getValue(Theta1[s+1])*(1-lambda)-CPX[s].getValue(Theta2[s+1])*lambda;
			...
			for(s = No_stage-1; s>0; s--){
                for(w=0; w<No_scenNode; w++){
					...
					IloNumArray z_w_copy(env, No_scen);
					...
					if(stageMap[w] == s-1){
						...
						//store the index of var 
						
						if (stageMap[w] == No_stage-2){
							
							for (j = 0; j<No_scen; j++){
								if (z_w[CVaR_No]==z_w_copy[j]){
									varIndex[w][nCut]=j;

									//VarIndex[w][nCut]=(w-No_scen-1)*No_scen+No_scen*No_scen+No_scen+j+1;
           ...
           for(s = No_stage-1; s>0; s--){
				MODEL[s-1].add(Cut1[s-1][nCut]);
				MODEL[s-1].add(Cut2[s-1][nCut]);
				
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						//cout<< "varIndex="<< varIndex[w][nCut]<< endl;
						//update the constraints right hand side using scenFac[w], storage_hat[w];
						//you have done that before
						for(k=0;k<nCut;k++){ 
							Cut1[s-1][k].setBounds(CutRhs1[w][k], IloInfinity);
							Cut2[s-1][k].setBounds(CutRhs2[w][k], IloInfinity);
							for(i=0;i<No_recharge; i++){ 
								Cut1[s-1][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs1[w][k][i]);
								Cut2[s-1][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs2[w][k][i]);
							}
						}//end for k
                       ...
                       for(t=0;t<No_year[s-1];t++){
						...
						//take care of the rest of g
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									g[w][j][nCut] += pi_Cut1[decendant[w][j]][k]*CutRhs1[decendant[w][j]][k];
									g[w][j][nCut]+=g[decendant[w][j]][varIndex[decendant[w][j]][k]][k]*pi_Cut2[decendant[w][j]][k];
									g[w][j][nCut] += psi[decendant[w][j]][k]*pi_Cut2[decendant[w][j]][k]/(1-alpha);
											
									for(i=0;i<No_recharge; i++){
										g[w][j][nCut] -= psi_x[decendant[w][j]][k][i]*pi_Cut2[decendant[w][j]][k]*storage_hat[i][decendant[w][j]][k]/(1-alpha);
										g[w][j][nCut] -= psi_eta[decendant[w][j]][k]*pi_Cut2[decendant[w][j]][k]*H[decendant[w][j]][k][i]*storage_hat[i][decendant[w][j]][k]/(1-alpha);
									}
								
								}
							}

							CutRhs1[w][nCut]+=prob*g[w][j][nCut];
							
						}//end j
						//take care of G
						for(i=0;i<No_recharge; i++){
							//CutLhs1[w][nCut][i]= 0;
							//H[w][nCut][i]=-pi_storage[VarIndex[w][nCut]][i]+ pi_flowBound[VarIndex[w][nCut]][i];
							for(j=0;j<No_scen;j++){
								
								G[w][j][nCut][i]=-pi_storage[decendant[w][j]][i]+ pi_flowBound[decendant[w][j]][i];
								if (j==varIndex[w][nCut]){
									H[w][nCut][i]=G[w][j][nCut][i];
								}
								CutCVaR[w][j]+=G[w][j][nCut][i]*storage_hat[i][w][nCut];
														
								CutLhs1[w][nCut][i] += prob*G[w][j][nCut][i];
								...
								for(j=0;j<No_scen;j++){
									CutCVaR[w][j]+=g[w][j][nCut];
								}
								for(j=0;j<No_scen;j++){
									if (CutCVaR[w][j]-CutCVaR[w][varIndex[w][nCut]] >0){
										psi[w][nCut]+=prob*(CutCVaR[w][j]-CutCVaR[w][varIndex[w][nCut]] );
										psi_eta[w][nCut]-=prob;
										for(i=0;i<No_recharge; i++){
											psi_x[w][nCut][i]+=G[w][j][nCut][i]*prob;
										}
									}
								}//end j (descendant nodes)
							
								for(i=0;i<No_recharge; i++){
									Cut1[s-1][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs1[w][nCut][i]); 

									
									CutLhs2[w][nCut][i]+=psi_x[w][nCut][i]/(1-alpha);
									CutLhs2[w][nCut][i]+=H[w][nCut][i];
									CutLhs2[w][nCut][i]+=psi_eta[w][nCut]*H[w][nCut][i]/(1-alpha);
									Cut2[s-1][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs2[w][nCut][i]); 
									
								}

						Cut1[s-1][nCut].setBounds(CutRhs1[w][nCut], IloInfinity);
						
						CutRhs2[w][nCut]+=psi[w][nCut]/(1-alpha);
						for(i=0;i<No_recharge; i++){
							CutRhs2[w][nCut]-=psi_x[w][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
							CutRhs2[w][nCut]-=psi_eta[w][nCut]*H[w][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
						}
						CutRhs2[w][nCut]+=g[w][varIndex[w][nCut]][nCut];
						
						Cut2[s-1][nCut].setBounds(CutRhs2[w][nCut], IloInfinity);
						...
						if(s<=No_stage-1 && s>1){
							CPX[s-1].solve();
							subObj_hat[w] = CPX[s-1].getObjValue();
							...
							//Dual value for cut1
							for(k=0;k<nCut+1;k++){
								pi_Cut1[w][k] =  CPX[s-1].getDual(Cut1[s-1][k]); 
								pi_Cut2[w][k] =  CPX[s-1].getDual(Cut2[s-1][k]); 
							}  
				...
				if (s>1){
					for (w2=0; w2< No_scenNode; w2++){
						if (stageMap[w2]==s-2){
							...
							IloNumArray temp(env, No_scen);
							for (j=0; j<No_scen; j++){
								z_w[j] = subObj_hat[decendant[w2][j]]; 
							}
							for(j=0; j< No_scen; j++)
								z_w_copy[j]=z_w[j];
							...
							for (j = 0; j<No_scen; j++){
								if (z_w[CVaR_No]==z_w_copy[j]){
									varIndex[w2][nCut]=j;
									/*if (stageMap[w2]==1)
										VarIndex[w2][nCut]=(w2-1)*No_scen+No_scen+j+1;
									if (stageMap[w2]==0)
										VarIndex[w2][nCut]=j+1;
									*/
								}
							}
						}
