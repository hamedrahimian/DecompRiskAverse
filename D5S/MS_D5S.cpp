...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if( s<No_stage-1){ 
				 for(j=0; j<No_scen; j++){
					OBJ[s] +=(1-lambda)*prob*Theta1[s+1][j] ; 
				 }
				 OBJ[s] +=lambda*Theta2[s+1]; 
			 }
...
//****
			 ...
			 if(s<No_stage-1){
				...
				for(nCut = 0; nCut<MaxCut; nCut++){
					...
					for(i=0;i<No_recharge; i++){ 
					 ...
					 Cut2[s].add(Theta2[s+1] - OptCutExpr2>= -IloInfinity );
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			for(j=0;j<No_scen;j++)
				subObj_hat[0] -=  (1-lambda)*prob*CPX[0].getValue(Theta1[1][j]); 

			subObj_hat[0]-= CPX[0].getValue(Theta2[1])*lambda;
            ...
            for(s=1; s<No_stage;s++){ 
			   ...
			   // 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue();
							for(j=0;j<No_scen;j++)
								subObj_hat[w] -=  (1-lambda)*prob*CPX[s].getValue(Theta1[s+1][j]); 

							subObj_hat[w]-= CPX[s].getValue(Theta2[s+1])*lambda;
                       ...
                       for(t=0;t<No_year[s-1];t++){
							...
							if (s<No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									for(w2 = 0; w2<No_scen; w2++){
										g[w][j][nCut] += pi_Cut1[decendant[w][j]][w2][k]*CutRhs1[decendant[w][j]][w2][k];	
									}
						...
						for(i=0;i<No_recharge; i++){
							...
							for(j=0;j<No_scen;j++){
								...
								CutLhs1[w][j][nCut][i] =G[w][j][nCut][i];
