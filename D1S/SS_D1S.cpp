...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 for(w2 = 0; w2<No_scenNode;w2++){
						if(stageMap[w2]==s+1){ 
							OptCutExpr2 += prob*V[nCut][w2];
						}
					}
					 
					 Cut2[s].add(Theta2[s+1] - OptCutExpr2 >= 0 );
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			subObj_hat[0] = CPX[0].getObjValue() - (1-lambda)*CPX[0].getValue(Theta1[1])- lambda*CPX[0].getValue(Theta2[1]);
					...
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
						       ...
						       //update cut2 
								Cut2[s][k].setBounds(0,IloInfinity);
								for(w2=0;w2<No_scenNode;w2++){
									if(ancestor[w][w2]==1)
										Cut2[s][k].setLinearCoef(V[k][w2], -prob/(1-alpha));
									else
										Cut2[s][k].setLinearCoef(V[k][w2], 0);
								}
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue() - (1-lambda)*CPX[s].getValue(Theta1[s+1])- lambda*CPX[s].getValue(Theta2[s+1]);
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							//update cut2 
							Cut2[s-1][k].setBounds(0,IloInfinity);
							for(w2=0;w2<No_scenNode;w2++){
								if(ancestor[w][w2]==1)
									Cut2[s-1][k].setLinearCoef(V[k][w2], -prob/(1-alpha));
								else
									Cut2[s-1][k].setLinearCoef(V[k][w2], 0);
							}
						...
						if (s< No_stage-1){
							for(k=0;k<nCut+1;k++){
								for(j=0; j<No_scen; j++){
									h[w][j][nCut] += pi_Cut1[decendant[w][j]][k]*CutRhs1[decendant[w][j]][k];
						...
						for(j=0; j<No_scen; j++){
							...
							CutRhs1[w][nCut] +=  prob*h[w][j][nCut];
						}
						//CutRhs1[w][nCut]=CutRhs1g[w][nCut];
						...
						for(i=0;i<No_recharge; i++){
							for(j=0; j<No_scen; j++){
								...
								CutLhs1[w][nCut][i]+= prob*CutLhs3[w][j][nCut][i];
						...
						Cut2[s-1][nCut].setBounds(0, IloInfinity);
						
						for(w2=0; w2<No_scenNode; w2++){ 
							if(ancestor[w][w2]==1){
								Cut2[s-1][nCut].setLinearCoef(V[nCut][w2],-prob/(1-alpha));
							}
							else{
								Cut2[s-1][nCut].setLinearCoef(V[nCut][w2],0);
							}
						}
