			 ...
			 if(s<No_stage-1){
				for(w = 0; w<No_scen;w++){
					...
					for(nCut = 0; nCut<MaxCut; nCut++){
						...
						for(w2 = 0; w2<No_scenNode;w2++){
							if(stageMap[w2]==s+1){ 
								OptCutExpr1 += V[nCut][w2];
							}
						}
						sprintf_s(name1, "Cut1_%d_%d_%d",(int) s, (int)w,(int) nCut);
						Cut1[s][w].add(Theta1[s+1][w] - lambda*VaR[nCut][s] - OptCutExpr1 >= -IloInfinity );
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								for(j=0;j<No_scen;j++){
									...
									for(w2=0;w2<No_scenNode;w2++){
										if(w2 == decendant[w][j]){
											Cut1[s][j][k].setLinearCoef(V[k][w2], -lambda/(1-alpha));
											Cut3[s][j][k].setLinearCoef(V[k][w2],1);
										}
										else{
											Cut1[s][j][k].setLinearCoef(V[k][w2],0);
					...
					if(stageMap[w]==s-1){ 
						for(k=0;k<nCut;k++){  
							for(j=0;j<No_scen;j++){
								...
								for(w2=0;w2<No_scenNode;w2++){
									if(w2 == decendant[w][j]){
										Cut1[s-1][j][k].setLinearCoef(V[k][w2], -lambda/(1-alpha));
										Cut3[s-1][j][k].setLinearCoef(V[k][w2],1);
									}
									else{
										Cut1[s-1][j][k].setLinearCoef(V[k][w2],0);
                       ...
                       //you have done that before
						...
						if (s<No_stage-1){
							for(j=0; j<No_scen; j++){ 
								for(k=0;k<nCut+1;k++){
									for(w2 = 0; w2<No_scen; w2++){
										...
										h[w][j][nCut] += pi_Cut1[decendant[w][j]][w2][k]*g[decendant[w][j]][w2][k]*(1-lambda);
						 ...
						 for(j=0; j<No_scen; j++){
							g[w][j][nCut]=h[w][j][nCut];
							CutRhs3[w][j][nCut]=h[w][j][nCut];
							CutRhs1[w][j][nCut]=(1-lambda)*g[w][j][nCut];
						...
						for(i=0;i<No_recharge; i++){
							for(j=0; j<No_scen; j++){
								...
								CutLhs1[w][j][nCut][i]=(1-lambda)*CutLhs3[w][j][nCut][i];
						...
						for(j=0; j<No_scen; j++){  
							for(w2=0;w2<No_scenNode;w2++){
								if(w2 == decendant[w][j]){
									Cut1[s-1][j][nCut].setLinearCoef(V[nCut][w2],-lambda/(1-alpha));
									Cut3[s-1][j][nCut].setLinearCoef(V[nCut][w2],1);
								}
								else{
									Cut1[s-1][j][nCut].setLinearCoef(V[nCut][w2],0);
