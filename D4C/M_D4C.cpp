...
//****
			 ...
			 if(s<No_stage-1){
				 for(w = 0; w<No_scen;w++){
					...
					for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 Cut1[s][w].add(Theta1[s+1][w] - lambda*VaR[s] - OptCutExpr1 >= -IloInfinity );
		...
		IloNumArray3 CutRhs1g(env,No_scenNode); 
		...
		for(w2=0; w2<No_scenNode; w2++){
			...
			CutRhs1g[w2] = IloNumArray2(env,No_scen); 
			...
			for(j = 0;j<No_scen;j++){
				...
				CutRhs1g[w2][j] = IloNumArray(env,MaxCut); 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								for(j=0;j<No_scen;j++){
									...
									Cut1[s][j][k].setLinearCoef(VaR[s], -lambda-lambda*psi_eta[w][j][k]/(1-alpha)); 
								}
							}//end for nCut
					...
					if(stageMap[w]==s-1){ 
						for(k=0;k<nCut;k++){ 
							for(j=0;j<No_scen;j++){
								...
								Cut1[s-1][j][k].setLinearCoef(VaR[s-1], -lambda-lambda*psi_eta[w][j][k]/(1-alpha)); 
							}
						}//end for nCut
							...
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									for(w2 = 0; w2<No_scen; w2++){
										g[w][j][nCut] += pi_Cut1[decendant[w][j]][w2][k]*g[decendant[w][j]][w2][k]*(1-lambda);
										g[w][j][nCut] += lambda*psi[decendant[w][j]][w2][k]*pi_Cut1[decendant[w][j]][w2][k]/(1-alpha);
										for(i=0;i<No_recharge; i++){
											g[w][j][nCut] -= lambda*psi_x[decendant[w][j]][w2][k][i]*pi_Cut1[decendant[w][j]][w2][k]*storage_hat[i][decendant[w][j]][k]/(1-alpha);
										}
										g[w][j][nCut] -= lambda*psi_eta[decendant[w][j]][w2][k]*pi_Cut1[decendant[w][j]][w2][k]*var[decendant[w][j]][k]/(1-alpha);
									}										
								}
							}
						...
						for(j=0;j<No_scen;j++){
							for(i=0;i<No_recharge; i++){
									CutLhs1[w][j][nCut][i] = CutLhs1[w][j][nCut][i]*(1-lambda);
									CutLhs1[w][j][nCut][i]+=lambda*psi_x[w][j][nCut][i]/(1-alpha);
									Cut1[s-1][j][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs1[w][j][nCut][i]); 
							}
							Cut1[s-1][j][nCut].setLinearCoef(VaR[s-1], -lambda-lambda*psi_eta[w][j][nCut]/(1-alpha)); 

							CutRhs1[w][j][nCut]=  g[w][j][nCut]*(1-lambda);
							CutRhs1[w][j][nCut]+=lambda*psi[w][j][nCut]/(1-alpha);
							for(i=0;i<No_recharge; i++){
								CutRhs1[w][j][nCut]-=lambda*psi_x[w][j][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
							}
							CutRhs1[w][j][nCut]-=lambda*psi_eta[w][j][nCut]*var[w][nCut]/(1-alpha);
