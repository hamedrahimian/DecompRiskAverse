		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 //???what is w???there is no loop on w!
					 for(w2 = 0; w2<No_scenNode;w2++){
							 //sum over all nodes in stage s+1`	
							 if(stageMap[w2]==s+1){ 
								 OptCutExpr1 += prob*V[nCut][w2];
							 }
					}
					
					 sprintf_s(name1, "Cut1_%d_%d",(int) s,(int) nCut);
					 //???
					 Cut1[s].add(Theta1[s+1] - lambda*VaR[nCut][s] - OptCutExpr1 >= -IloInfinity );
	            ...
	            for(w=1; w<No_scenNode; w++){
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								//why changing all previous cuts??!! see later
								...
								for(w2=0;w2<No_scenNode;w2++){
									if(ancestor[w][w2]==1)
										Cut1[s][k].setLinearCoef(V[k][w2], -prob*lambda/(1-alpha));
									else
										//since it has added unnecessary variables to cut1, it must change their coefficient to zero
										Cut1[s][k].setLinearCoef(V[k][w2], 0);
								} 
						...
						if(s==No_stage-1){
							...
							/*cout<<"subObj_hat["<<w<<"]="<<subObj_hat[w]<<endl;*/
							for(t=0;t<No_year[s];t++){ 
								for(i=0; i<No_capacity; i++) {
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							for(w2=0;w2<No_scenNode;w2++){
								if(ancestor[w][w2]==1)
									Cut1[s-1][k].setLinearCoef(V[k][w2], -prob*lambda/(1-alpha));
								else
									Cut1[s-1][k].setLinearCoef(V[k][w2], 0);
							}
                       ...
                       //you have done that before
						...
						if (s<No_stage-1){
							for(k=0;k<nCut+1;k++){
								for(j=0; j<No_scen; j++){
									h[w][j][nCut] += (1-lambda)*pi_Cut1[decendant[w][j]][k]*CutRhs1g[decendant[w][j]][k];
									for(w2 = 0; w2<No_scen; w2++){
										h[w][j][nCut] += pi_Cut3[decendant[w][j]][w2][k]*h[decendant[w][j]][w2][k];
						...
						for(j=0; j<No_scen; j++){
							...
							CutRhs1g[w][nCut] +=  prob*h[w][j][nCut];
						}
						CutRhs1[w][nCut]=(1-lambda)*CutRhs1g[w][nCut];
						...
						for(i=0;i<No_recharge; i++){
							...
							CutLhs1[w][nCut][i]=(1-lambda)*CutLhs1[w][nCut][i];
						...
						for(w2=0; w2<No_scenNode; w2++){ 
							if(ancestor[w][w2]==1){
								Cut1[s-1][nCut].setLinearCoef(V[nCut][w2],-prob*lambda/(1-alpha));
							}
							else{
								Cut1[s-1][nCut].setLinearCoef(V[nCut][w2],0);
							}
						}
