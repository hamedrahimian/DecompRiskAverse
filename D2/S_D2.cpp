...
///******
 ...
 /////////////////// DECISION VARIABLES WITH NAMES  /////////////////////////////
		...
		IloNumVarArray  V(env, No_stage, 0, IloInfinity, ILOFLOAT);
		...
		IloNumArray var(env,No_scenNode);
		for(w=0;w<No_scenNode;w++)
			var[w] = 0;

        //Dual values 
		IloNumArray3 pi_balance(env, No_yearMax);  //dual variable for demand satisfication constraints
		...
		IloNumArray2 pi_storage(env, No_scenNode);  //dual variable for storage constraints
		IloNumArray2 pi_flowBound(env, No_scenNode);  //dual variable for RF outflow bound constraints 
        IloNumArray  pi_cvar(env,No_scenNode); //dual varible for the cvar constriants
...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
             ...
             if(s<No_stage-1){ 
				 CVaRCons.add(V[s] - OBJ[s] -  lambda*VaR[s] - Theta1[s+1] >= -IloInfinity);
			 }
			 else{
				 CVaRCons.add(V[s] - OBJ[s]  >= -IloInfinity);
			 }
			 ...
			 if(s>0 && s<No_stage-1){
				 ...
				 OBJ[s] = OBJ[s]*(1-lambda);
				 OBJ[s] += lambda*V[s]/(1-alpha);
			 }
			 else if(s == No_stage -1){
			 ...
			 for(t = 0;t<No_yearMax;t++){
                 ...
                 for(i=0;i<No_recharge; i++){ // recharge facility storage upper bound
                 ...
                 for(i=0;i<No_potUser; i++){  
					...
					IloExpr LHS(env);
					IloNum RHS;	 
					RHS = returnRate*demand[s][t][i];                // 98% of the potable uses retuern to the facility
					for(j=0;j<No_link;j++){
                 ...
                 for(i=0;i<No_recharge; i++){                  
					...
					if( t == 0){
						//Storage[i][t] - Storage0[i] - inflow  = - outflow
						LHS += - Storage[i][s][t] ;
						if(s==0){ 
						...
						//Storage[i][t] - Storage[i][t-1] - inflow = - outflow
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 Cut1[s].add(Theta1[s+1] + OptCutExpr1 + VaR[s]>= -IloInfinity );
		...
		IloNumArray2 CutLhsV(env,No_scenNode);
		for(w2=0; w2<No_scenNode; w2++){
			CutRhs1[w2] = IloNumArray(env,MaxCut); 
			CutLhs1[w2] = IloNumArray2(env,MaxCut);
			CutLhsV[w2] = IloNumArray(env,MaxCut);
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			var[0]=CPX[0].getValue(VaR[0]);
			
			LB = CPX[0].getObjValue(); 
			 
			//subObj_cx[0] = CPX[0].getObjValue() - CPX[0].getValue(Theta1[1])- lambda*CPX[0].getValue(VaR[0]);
            ...
            for(s=1; s<No_stage;s++){ 
	            ...
	            for(w=1; w<No_scenNode; w++){
					if(stageMap[w]==s){ 
						...
						for(w2=0; w2<No_scenNode; w2++){
							if(ancestor[w2][w]==1){
							   CVaRCons[s].setBounds(-var[w2],IloInfinity);
							   for(i=0;i<No_recharge; i++){
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								Cut1[s][k].setLinearCoef(VaR[s], CutLhsV[w][k]);
								for(i=0;i<No_recharge; i++){ 
						...
						if(s<No_stage-1)
							var[w] = CPX[s].getValue(VaR[s]);	
						if(s==No_stage-1){
							//subObj_hat[w] = CPX[s].getObjValue();
							subObj_hat[w] = CPX[s].getObjValue() - lambda*CPX[s].getValue(V[s])/(1-alpha);
							subObj_hat[w] = subObj_hat[w]/(1-lambda);
							...
							pi_Cut1[w][nCut] = 0;
							pi_cvar[w] = CPX[s].getDual(CVaRCons[s]);
						}
						else{    
							subObj_hat[w] = CPX[s].getObjValue() - lambda*CPX[s].getValue(V[s])/(1-alpha);
							subObj_hat[w] = subObj_hat[w]/(1-lambda);
							subObj_hat[w] -= CPX[s].getValue(Theta1[s+1]);
							//subObj_cx[w] = subObj_hat[w] - lambda*CPX[s].getValue(VaR[s]);
				...
				for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){
						for(j = 0; j<No_scen; j++){
						   ...
						   if(subObj_hat[decendant[w][j]] - var[w]>=0){
							   subObj_hat[w] +=prob*lambda*(subObj_hat[decendant[w][j]] - var[w])/(1-alpha);
						   }
						}
					}
				}
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							Cut1[s-1][k].setLinearCoef(VaR[s-1], CutLhsV[w][k]);
							for(i=0;i<No_recharge; i++){ 
						...
						for(w2=0; w2<No_scenNode; w2++){
							if(s-1>0){
								if(ancestor[w2][w]==1){
								   CVaRCons[s-1].setBounds(-var[w2],IloInfinity);
								   for(i=0;i<No_recharge; i++){
						...
						if (s< No_stage-1){
							for(j=0; j<No_scen; j++){ 
								for(k=0;k<nCut+1;k++){ 
									CutRhs1[w][nCut] += prob*pi_Cut1[decendant[w][j]][k]*CutRhs1[decendant[w][j]][k];
								} 
							}
						}
					   ...
					   for(j=0; j<No_scen; j++){
						    for(t =0; t<No_year[s];t++){
				               for(i=0; i<No_user; i++){
					              CutRhs1[w][nCut] += prob*pi_demand[t][decendant[w][j]][i]*scenFac[decendant[w][j]][t+1]*demand[s][t][i];
							   }
				               for(i=0; i<No_potUser; i++){
					              CutRhs1[w][nCut] += prob*pi_return[t][decendant[w][j]][i]*returnRate*scenFac[decendant[w][j]][t+1]*demand[s][t][i];
							   }
						       for(i=1;i<No_capacity; i++){ 
					              CutRhs1[w][nCut] += prob*pi_capacity[t][decendant[w][j]][i]*capacity[i];
							   }
							   CutRhs1[w][nCut] += prob*pi_capacity[t][decendant[w][j]][0]*CAPamt[s][t]*scenFac[decendant[w][j]][0];
							   for(i=0;i<No_recharge; i++){ 
					              CutRhs1[w][nCut] += prob*pi_safe[t][decendant[w][j]][i]*storageUB[i];
							   }
							}
					   } 
						...
						for(i=0;i<No_recharge; i++){
							for(j=0; j<No_scen; j++){    
								CutLhs1[w][nCut][i] += prob*(-pi_storage[decendant[w][j]][i]+pi_flowBound[decendant[w][j]][i]);
							}
                        ...
                        for(j=0; j<No_scen; j++){    
							CutLhsV[w][nCut] += prob*pi_cvar[decendant[w][j]];
						}
						
						Cut1[s-1][nCut].setLinearCoef(VaR[s-1], CutLhsV[w][nCut]); 
						...
						if(s<=No_stage-1 && s>1){
							...
							pi_cvar[w] = CPX[s-1].getDual(CVaRCons[s-1]);
						}	
					} 
				}
			}//end backward  
