...
///////////////////read demeand, capacity, and intial storage//////////////////
		  ...
		  IloInt No_scen =25; //Number of scenarios in each stage
		...
		const char* filename4  =  "Data\\scenFac_25.txt";
...
///******
 ...
 /////////////////// DECISION VARIABLES WITH NAMES  /////////////////////////////
		...
		//****
		IloNumVarArray	VaR(env,No_stage, 0, IloInfinity, ILOFLOAT);
		//*****
		...
		//****
		IloNumArray2 var(env,No_scenNode);
		for(w=0;w<No_scenNode;w++){
			var[w] = IloNumArray(env, MaxCut);
			for(nCut=0;nCut<MaxCut;nCut++){
				var[w][nCut] = 0;
			}
		}
      ...
      //****
		...
		IloNumArray3  pi_Cut2(env, No_scenNode);
		
		
        for(w=0;w<No_scenNode;w++){
			pi_Cut2[w] = IloNumArray2(env,No_scen);
			pi_Cut1[w] = IloNumArray(env,MaxCut);
			for(nCut=0;nCut<MaxCut;nCut++){
				pi_Cut1[w][nCut] = 0;
			}
			for(j=0;j<No_scen;j++){
				pi_Cut2[w][j] = IloNumArray(env,MaxCut);
				for(nCut=0;nCut<MaxCut;nCut++){
					pi_Cut2[w][j][nCut] = 0;
				}
			}
...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if( s<No_stage-1){ 
				 for(j=0; j<No_scen; j++){
					OBJ[s] +=lambda*prob*Theta2[s+1][j];
				 }
				 OBJ[s] +=(1-lambda)*Theta1[s+1];
...
//****
			 ...
			 if(s<No_stage-1){
				 for(w = 0; w<No_scen;w++){
					...
					for(nCut = 0; nCut<MaxCut; nCut++){
						...
						for(i=0;i<No_recharge; i++){ 
						 OptCutExpr2 += Storage[i][s][No_year[s]-1]; 
						}
					
						sprintf_s(name1, "Cut2_%d_%d_%d",(int) s, (int)w,(int) nCut);			
						Cut2[s][w].add(Theta2[s+1][w] - VaR[s] - OptCutExpr2 >= -IloInfinity );
		...
		for(w2=0; w2<No_scenNode; w2++){
			...
			for(j = 0;j<No_scen;j++){
				...
				CutRhs2[w2][j] = IloNumArray(env,MaxCut); 
				//CutLhs2[w2][j] = IloNumArray2(env,MaxCut);
				CutCVaR[w2][j] = 0;
				G[w2][j] = IloNumArray2(env,MaxCut);
				g[w2][j] = IloNumArray(env,MaxCut);
				
				psi[w2][j]=IloNumArray(env, MaxCut);
				psi_x[w2][j]=IloNumArray2(env, MaxCut);
				psi_eta[w2][j]=IloNumArray(env, MaxCut);
				...
				for(nCut=0;nCut<MaxCut;nCut++){
					...
					CutRhs1[w2][nCut] = 0; 
					psi[w2][j][nCut]=0;
					//CutLhs2[w2][j][nCut] = IloNumArray(env,No_recharge);
					psi_x[w2][j][nCut]=IloNumArray(env,No_recharge);
					psi_eta[w2][j][nCut]=0;
					CutLhs1[w2][nCut] = IloNumArray(env,No_recharge);
					for(i=0;i<No_recharge;i++){
						G[w2][j][nCut][i]=0;
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			if (nCut >0)
				var[0][nCut]=CPX[0].getValue(VaR[0]);
			...
			for(j=0;j<No_scen;j++){
				subObj_hat[0]-= prob*CPX[0].getValue(Theta2[1][j])*lambda;
			}
			subObj_hat[0]-=(1-lambda)*CPX[0].getValue(Theta1[1]); 
            ...
            for(s=1; s<No_stage;s++){ 
					...
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								for(j=0;j<No_scen;j++){
									Cut2[s][j][k].setBounds(CutRhs2[w][j][k], IloInfinity);
									for(i=0;i<No_recharge; i++){ 
										Cut2[s][j][k].setLinearCoef(Storage[i][s][No_year[s]-1], -psi_x[w][j][k][i]/(1-alpha));

									}
									Cut2[s][j][k].setLinearCoef(VaR[s], -1-psi_eta[w][j][k]/(1-alpha)); 
								}
			   ...
			   // 
						...
						if(nCut>0 && s<No_stage-1)
							var[w][nCut]= CPX[s].getValue(VaR[s]);
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue();
							for(j=0;j<No_scen;j++){
								subObj_hat[w]-= prob*CPX[s].getValue(Theta2[s+1][j])*lambda;
							}
							subObj_hat[w]-=(1-lambda)*CPX[s].getValue(Theta1[s+1]); 
			...
			for(s = No_stage-1; s>0; s--){
				for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){
						subObj_hat[w] +=lambda*var[w][nCut];
						for(j = 0; j<No_scen; j++){
						   subObj_hat[w] +=  prob*subObj_hat[decendant[w][j]]*(1-lambda);
						   if(subObj_hat[decendant[w][j]] - var[w][nCut]>=0){
							   subObj_hat[w] +=prob*lambda*(subObj_hat[decendant[w][j]] - var[w][nCut])/(1-alpha);
						   }
						}
					}
				}
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
								...
								for(j=0;j<No_scen;j++){
									Cut2[s-1][j][k].setBounds(CutRhs2[w][j][k], IloInfinity);
									for(i=0;i<No_recharge; i++){ 
										Cut2[s-1][j][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][j][k][i]/(1-alpha));

									}
									Cut2[s-1][j][k].setLinearCoef(VaR[s-1], -1-psi_eta[w][j][k]/(1-alpha)); 
								}
                       ...
                       //you have done that before
							...
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									...
									for(w2 = 0; w2<No_scen; w2++){
										g[w][j][nCut] += psi[decendant[w][j]][w2][k]*pi_Cut2[decendant[w][j]][w2][k]/(1-alpha);
										for(i=0;i<No_recharge; i++){
											g[w][j][nCut] -= psi_x[decendant[w][j]][w2][k][i]*pi_Cut2[decendant[w][j]][w2][k]*storage_hat[i][decendant[w][j]][k]/(1-alpha);
										}
										g[w][j][nCut] -= psi_eta[decendant[w][j]][w2][k]*pi_Cut2[decendant[w][j]][w2][k]*var[decendant[w][j]][k]/(1-alpha);
									}
								}
							}
						...
						for(j=0;j<No_scen;j++){
							...
							if (CutCVaR[w][j]-var[w][nCut] >0){
								psi[w][j][nCut]=CutCVaR[w][j]-var[w][nCut];
								psi_eta[w][j][nCut]=-1;
								for(i=0;i<No_recharge; i++){
									psi_x[w][j][nCut][i]=G[w][j][nCut][i];
								}
							}
						...
						for(j=0;j<No_scen;j++){
							for(i=0;i<No_recharge; i++){
									Cut2[s-1][j][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][j][nCut][i]/(1-alpha)); 
							}			
							Cut2[s-1][j][nCut].setLinearCoef(VaR[s-1], -1-psi_eta[w][j][nCut]/(1-alpha)); 
						}
						

						for(j=0;j<No_scen;j++){
							CutRhs2[w][j][nCut]=psi[w][j][nCut]/(1-alpha);
							for(i=0;i<No_recharge; i++){
								CutRhs2[w][j][nCut]-=psi_x[w][j][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
							}
							CutRhs2[w][j][nCut]-=psi_eta[w][j][nCut]*var[w][nCut]/(1-alpha);
							Cut2[s-1][j][nCut].setBounds(CutRhs2[w][j][nCut], IloInfinity);
						...
						if(s<=No_stage-1 && s>1){
							...
							for(k=0;k<nCut+1;k++){
								for(j=0;j<No_scen;j++){
								   pi_Cut2[w][j][k] =  CPX[s-1].getDual(Cut2[s-1][j][k]); 
								}
							}
