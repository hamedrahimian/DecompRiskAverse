...
////////////BUILD MASTER AND SUBPROBLEM MODELS  //////////////////////////
		...
		//Constraints array
		 ...
		 for(s = 0; s<No_stage; s++){
			 ...
			 if( s<No_stage-1){ 
				 for(j=0; j<No_scen; j++){
					OBJ[s] +=lambda*prob*Theta2[s+1][j]/(1-alpha);
				 }
...
//****
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			for(j=0;j<No_scen;j++){
				subObj_hat[0]-= prob*CPX[0].getValue(Theta2[1][j])*lambda/(1-alpha);
			}
            ...
            for(s=1; s<No_stage;s++){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								for(j=0;j<No_scen;j++){
									...
									for(i=0;i<No_recharge; i++){ 
										Cut2[s][j][k].setLinearCoef(Storage[i][s][No_year[s]-1], -psi_x[w][j][k][i]);

									}
									Cut2[s][j][k].setLinearCoef(VaR[s], -psi_eta[w][j][k]); 
								}
			   ...
			   // 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue();
							for(j=0;j<No_scen;j++){
								subObj_hat[w]-= prob*CPX[s].getValue(Theta2[s+1][j])*lambda/(1-alpha);
							}
					...
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
								...
								for(j=0;j<No_scen;j++){
									...
									for(i=0;i<No_recharge; i++){ 
										Cut2[s-1][j][k].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][j][k][i]);

									}
									Cut2[s-1][j][k].setLinearCoef(VaR[s-1], -psi_eta[w][j][k]); 
								}
                       ...
                       //you have done that before
							...
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									...
									for(w2 = 0; w2<No_scen; w2++){
										g[w][j][nCut] += psi[decendant[w][j]][w2][k]*pi_Cut2[decendant[w][j]][w2][k];
										for(i=0;i<No_recharge; i++){
											g[w][j][nCut] -= psi_x[decendant[w][j]][w2][k][i]*pi_Cut2[decendant[w][j]][w2][k]*storage_hat[i][decendant[w][j]][k];
										}
										g[w][j][nCut] -= psi_eta[decendant[w][j]][w2][k]*pi_Cut2[decendant[w][j]][w2][k]*var[decendant[w][j]][k];
									}
								}
							}
						...
						for(j=0;j<No_scen;j++){
							for(i=0;i<No_recharge; i++){
									...
									Cut2[s-1][j][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -psi_x[w][j][nCut][i]); 
							}			
							Cut2[s-1][j][nCut].setLinearCoef(VaR[s-1], -psi_eta[w][j][nCut]); 
						}
						

						for(j=0;j<No_scen;j++){
							CutRhs2[w][j][nCut]=psi[w][j][nCut];
							for(i=0;i<No_recharge; i++){
								CutRhs2[w][j][nCut]-=psi_x[w][j][nCut][i]*storage_hat[i][w][nCut];
							}
							CutRhs2[w][j][nCut]-=psi_eta[w][j][nCut]*var[w][nCut];
