...
//****
			 ...
			 if(s<No_stage-1){
				 for(nCut = 0; nCut<MaxCut; nCut++){
					 ...
					 //it needs storage[Storage[i][s][No_year[s]-1] to pass it to next stage
					 //we care about constraints involving ancestor, and note these constraints which are StorageBalance and RFoutflowBound
					 //does not have rhs (say b), so there is no need to \pi*b
					 for(i=0;i<No_recharge; i++){ 
					 ...
					 Cut1[s].add(Theta1[s+1] - lambda*VaR[s] - OptCutExpr1 >= -IloInfinity );
		...
		for(w2=0; w2<No_scenNode; w2++){
			...
			for(nCut=0;nCut<MaxCut;nCut++){
				//it must change later to \pi*b for constraints not involving ancestor
				CutRhs1[w2][nCut] = 0; 
				CutRhs1g[w2][nCut] = 0; 
				psi[w2][nCut]=0;
				...
				for(i=0;i<No_recharge;i++){
					//it must change later to (1-lambda)*G(i) (G is corresponding dual variable for (4.19 minus 4.23)
					CutLhs1[w2][nCut][i] = 0;
					psi_x[w2][nCut][i]=0;
				}
			}
        ...
        for(nCut = 0; nCut<MaxCut; nCut++){ //main loop
			...
			subObj_hat[0] = CPX[0].getObjValue() - CPX[0].getValue(Theta1[1]);
            ...
            for(s=1; s<No_stage;s++){ 
					...
					if(stageMap[w]==s){ 
						...
						if(s<No_stage-1 && nCut>0){ 
							for(k=0;k<nCut;k++){ 
								...
								Cut1[s][k].setLinearCoef(VaR[s], -lambda-lambda*psi_eta[w][k]/(1-alpha)); 
			   ...
			   // 
						...
						if(s==No_stage-1){
							...
							subObj_hat[w] = CPX[s].getObjValue() - CPX[s].getValue(Theta1[s+1]);
                ...
                for(w=0; w<No_scenNode; w++){
					if(stageMap[w]==s-1){ 
						...
						for(k=0;k<nCut;k++){ 
							...
							Cut1[s-1][k].setLinearCoef(VaR[s-1], -lambda-lambda*psi_eta[w][k]/(1-alpha)); 	 
						}//end for k
						...
						for(j=0;j<No_scen;j++){
							...
							if (s< No_stage-1){
								for(k=0;k<nCut+1;k++){ 
									g[w][j][nCut] += pi_Cut1[decendant[w][j]][k]*CutRhs1g[decendant[w][j]][k]*(1-lambda);
									g[w][j][nCut] += lambda*psi[decendant[w][j]][k]*pi_Cut1[decendant[w][j]][k]/(1-alpha);
									for(i=0;i<No_recharge; i++){
										g[w][j][nCut] -= lambda*psi_x[decendant[w][j]][k][i]*pi_Cut1[decendant[w][j]][k]*storage_hat[i][decendant[w][j]][k]/(1-alpha);
									}
									g[w][j][nCut] -= lambda*psi_eta[decendant[w][j]][k]*pi_Cut1[decendant[w][j]][k]*var[decendant[w][j]][k]/(1-alpha);
						...
						for(i=0;i<No_recharge; i++){
							CutLhs1[w][nCut][i] = CutLhs1[w][nCut][i]*(1-lambda);
							CutLhs1[w][nCut][i]+=lambda*psi_x[w][nCut][i]/(1-alpha);
							Cut1[s-1][nCut].setLinearCoef(Storage[i][s-1][No_year[s-1]-1], -CutLhs1[w][nCut][i]); 
						}

						Cut1[s-1][nCut].setLinearCoef(VaR[s-1], -lambda-lambda*psi_eta[w][nCut]/(1-alpha)); 

						CutRhs1[w][nCut]+=  CutRhs1g[w][nCut]*(1-lambda);
						
						CutRhs1[w][nCut]+=lambda*psi[w][nCut]/(1-alpha);
						for(i=0;i<No_recharge; i++){
							CutRhs1[w][nCut]-=lambda*psi_x[w][nCut][i]*storage_hat[i][w][nCut]/(1-alpha);
						}
						CutRhs1[w][nCut]-=lambda*psi_eta[w][nCut]*var[w][nCut]/(1-alpha);
